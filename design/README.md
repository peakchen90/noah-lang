...
默认函数参数以及赋值都是传递值的拷贝，如果想传递值的内存引用，在类型前加 `&` 修饰

```hera
fn foo(name: &str) {
    // `name` 是引用类型，对 `name` 赋值会修改调用方的内存值 
    // (`main` 函数里的变量 `abc` 会被修改成 "123" )
    name = "123"
    return true
}
...
```hera
let a = 123
let b = a; // 内存拷贝，修改 b 不会影响 a
let c: &num = a // 内存引用，修改 c 同时也会修改 a 的值
